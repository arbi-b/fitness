...
export default async function Home({
  ...
  const all = await listPosts({ category: effectiveCategory === 'all' ? undefined : effectiveCategory, query: q, sort: 'newest' });

  // Popular: demo logic (tagged 'popular' first; fallback to newest)
  const popular = tab === 'popular' ? all.filter((p) => (p.tags || []).some((t) => t.toLowerCase() === 'popular')) : [];
  const feedSource = tab === 'popular' && popular.length ? popular : all;

  const pageSize = 5;
  const totalPages = Math.max(1, Math.ceil(feedSource.length / pageSize));
  const start = (page - 1) * pageSize;
  const feed = feedSource.slice(start, start + pageSize);
